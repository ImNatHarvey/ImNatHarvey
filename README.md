### Hi there! 👋

## I'm Josh Harvey Crisologo.

<p align="left">
  <a href="https://www.linkedin.com/in/imharveycrisologo/">
    <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white"/> </a>

  <a href="mailto:jharveycrisologo@gmail.com"> <img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white"/> </a>

  <a href="https://github.com/ImNatHarvey"> <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white"/> </a>

  <a href=""> <img src="https://img.shields.io/badge/Resume-4285F4?style=for-the-badge&logo=googledrive&logoColor=white"/> </a>
</p>

### 💻 About Me

- 🎓 BS Information Technology Student
- 🤖 Building AI-powered applications, automation workflows, and business management systems
- 🚀 Interested in Full-Stack Development, Artificial Intelligence, System Architecture, and Automation

---

## 📂 Featured Projects

### 🎓 [Study Aura](https://github.com/ImNatHarvey/Study-Aura)

> *An AI-powered learning ecosystem that transforms any source into personalized educational experiences.*

Study Aura is a modern educational platform that leverages AI automation pipelines to transform PDFs, websites, YouTube videos, images, and text into interactive learning materials. Designed for students and lifelong learners, it combines intelligent content generation, AI-powered study tools, and gamified learning experiences within a unified ecosystem.

* **Tech Stack:** React, TypeScript, Supabase, n8n, Gemini AI, Vite.
* **Key Features:**

  * 🤖 **AI Studio:** Generate quizzes, flashcards, mind maps, presentation slides, tables, and audio overviews from uploaded content.
  * 📚 **Multi-Source Learning:** Supports PDFs, websites, images, YouTube videos, web searches, and text-based sources.
  * 🎮 **Gamification System:** XP progression, activity tracking, learning milestones, and personalized educational experiences.
  * 🔄 **AI Automation Pipelines:** Powered by n8n workflows for content extraction, processing, summarization, and educational content generation.

<p>
  <img src="https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react&logoColor=61DAFB" height="20">&nbsp;
  <img src="https://img.shields.io/badge/TypeScript-3178C6?style=for-the-badge&logo=typescript&logoColor=white" height="20">&nbsp;
  <img src="https://img.shields.io/badge/Supabase-3ECF8E?style=for-the-badge&logo=supabase&logoColor=white" height="20">&nbsp;
  <img src="https://img.shields.io/badge/n8n-FF6D5B?style=for-the-badge&logo=n8n&logoColor=white" height="20">
</p>

---

### 🚗 [Nataraki Car Rental](https://github.com/ImNatHarvey/Nataraki-Car-Rental)

> *A comprehensive fleet scheduling, rental transaction, and business management platform.*

A Windows-based enterprise solution developed for Nataraki Car Rental to streamline fleet operations, reservation management, maintenance tracking, financial processing, customer records, and business analytics. The system centralizes day-to-day rental operations into a single management platform.

* **Tech Stack:** C#, WinForms, Microsoft SQL Server, Dapper, FluentValidation, BCrypt.Net.
* **Key Features:**

  * 📅 **Fleet Scheduling:** Reservation conflict prevention, maintenance scheduling, and vehicle availability management.
  * 💰 **Transaction Management:** Rental processing, payment tracking, invoice generation, and return inspections.
  * 🔐 **RBAC Security:** Role-based access control, audit logging, and permission management.
  * 📊 **Business Analytics:** Revenue reporting, fleet utilization monitoring, maintenance tracking, and operational insights.

<p>
  <img src="https://img.shields.io/badge/C%23-239120?style=for-the-badge&logo=c-sharp&logoColor=white" height="20">&nbsp;
  <img src="https://img.shields.io/badge/WinForms-512BD4?style=for-the-badge" height="20">&nbsp;
  <img src="https://img.shields.io/badge/SQL_Server-CC2927?style=for-the-badge&logo=microsoft-sql-server&logoColor=white" height="20">&nbsp;
  <img src="https://img.shields.io/badge/Dapper-007ACC?style=for-the-badge" height="20">
</p>

---

### 🥟 [MK Toasted Siopao Inventory System](https://github.com/ImNatHarvey/MK-Toasted-Siopao)

> *A full-stack ordering and inventory management system.*

A comprehensive web application designed to streamline business operations for a food business. It features a dual-interface for customers and administrators, deployed live for demonstration purposes.

* **Tech Stack:** Java 17, Spring Boot 3, Thymeleaf, MySQL, H2, Docker.
* **Key Features:**

  * 🛒 **Customer Module:** Online ordering with cart system, GCash receipt upload, and order tracking.
  * 🛡️ **Admin Dashboard:** Real-time analytics, recipe-based inventory deduction, and report generation.
  * 🚀 **Deployment:** Containerized with Docker and deployed on Render.

<p>
  <img src="https://img.shields.io/badge/Spring_Boot-6DB33F?style=for-the-badge&logo=spring-boot&logoColor=white" height="20">&nbsp;
  <img src="https://img.shields.io/badge/Thymeleaf-005F0F?style=for-the-badge&logo=thymeleaf&logoColor=white" height="20">&nbsp;
  <img src="https://img.shields.io/badge/MySQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white" height="20">&nbsp;
  <img src="https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white" height="20">
</p>

---

### 🎮 [Battle Hangpies: Integrated Gaming & Marketplace](https://github.com/ImNatHarvey/Battle-Hangpies)

> *A retro-inspired RPG battle engine with a modular marketplace system.*

An interactive Java application combining strategic word-puzzle combat with RPG progression mechanics and a simulated economy. Players collect Hangpies, engage in tactical battles, and manage resources through an integrated marketplace.

* **Tech Stack:** Java, AWT/Swing, OOP Design Patterns, File-Based Persistence.
* **Key Features:**

  * ⚔️ **Battle Arena:** Custom animations, sprite systems, and combat mechanics.
  * 🧩 **Word-Puzzle Combat:** Hangman-inspired battle system with strategic gameplay.
  * 💎 **Progression System:** EXP scaling, stat growth, and world-level progression.
  * 🏪 **Marketplace Module:** Buy, sell, and manage inventory using in-game currency.

<p>
  <img src="https://img.shields.io/badge/Java-ED8B00?style=for-the-badge&logo=openjdk&logoColor=white" height="20">&nbsp;
  <img src="https://img.shields.io/badge/Swing-blue?style=for-the-badge" height="20">&nbsp;
  <img src="https://img.shields.io/badge/OOP-Architecture-red?style=for-the-badge" height="20">&nbsp;
  <img src="https://img.shields.io/badge/Game_Design-purple?style=for-the-badge" height="20">
</p>

---

## 🚀 Skills & Dev Stack

<div align="center">

### Languages

<img src="https://skillicons.dev/icons?i=java,cs,ts,js" />

### Frontend

<img src="https://skillicons.dev/icons?i=react,tailwind,bootstrap,html,css,vite" />

### Backend & Frameworks

<img src="https://skillicons.dev/icons?i=spring,nodejs" />

### Database & Cloud

<img src="https://skillicons.dev/icons?i=mysql,supabase,docker" />

### AI & Automation

<img src="https://img.shields.io/badge/ChatGPT-74AA9C?style=flat-square&logo=openai&logoColor=white">
<img src="https://img.shields.io/badge/Claude-D97757?style=flat-square&logo=anthropic&logoColor=white">
<img src="https://img.shields.io/badge/Gemini_AI-4285F4?style=flat-square&logo=google-gemini&logoColor=white">
<img src="https://img.shields.io/badge/Mistral_AI-FF7000?style=flat-square&logo=mistral-ai&logoColor=white">
<img src="https://img.shields.io/badge/n8n-FF6D5B?style=flat-square&logo=n8n&logoColor=white">

### Tools & Development

<img src="https://skillicons.dev/icons?i=git,github,vscode,visualstudio,maven" />

</div>

---

## 📈 My GitHub Stats

<br>

<table border="0" align="center">
<tr border="0">
<td width="50%" align="center">
    <img align="center" src="https://github-readme-stats.vercel.app/api?username=ImNatHarvey&theme=cobalt&show_icons=true&count_private=true" />
  <br><br>
    <img align="center" src="https://github-readme-stats.anuraghazra1.vercel.app/api/top-langs/?username=ImNatHarvey&theme=dark&hide_border=true&no-bg=true&no-frame=true&layout=compact&langs_count=10"/>
  
</td>

<td width="50%" align="center">
    <img title="🔥 Get streak stats for your profile at git.io/streak-stats" alt="Mark streak" src="https://github-readme-streak-stats.herokuapp.com/?user=ImNatHarvey&theme=dark&hide_border=true" />

</td>
</tr>
</table>

<br>

<div align="center">
  <img alt="github contribution grid snake animation" src="https://github.com/ImNatHarvey/ImNatHarvey/blob/output/github-snake-dark.svg" />
</div>
